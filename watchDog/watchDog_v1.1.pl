...
sub readTaskList
{
    ...
    while (<IN>)
    {
        ...
        my $taskSh = sprintf("$scptDir/task_%04d.sh", $taskSeq);

        if (-e $taskSh && $numInSh == 0){
		unlink $taskSh;
	}
	if($numInSh == 0){
...
########  fix bug for shell list line number can't be divided by  $num_line  ###  20180802 
    my $taskSh = sprintf("$scptDir/task_%04d.sh", $taskSeq);
    unlink $taskSh if (-e $taskSh && $numInSh == 0); 
    push @arr_taskSh, $taskSh if(-f $taskSh); 
################################################################################
    return @arr_taskSh;
}
