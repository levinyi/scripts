...
def local_align(x, y, score=ScoreParam(10, -5, -7)):
    ...
    for i in xrange(1, len(x) + 1):
        for j in xrange(1, len(y) + 1):
            ...
            A[i][j] = max(A[i][j - 1] + score.gap, A[i - 1][j] + score.gap, A[i - 1][j - 1] + score.matchchar(x[i - 1], y[j - 1]), 0)
...
def affine_align(x, y, score=ScoreParam(10, -2, -7, -15)):
    ...
    for i in xrange(1, len(x) + 1):
        for j in xrange(1, len(y) + 1):
            M[i][j] = score.matchchar(x[i - 1], y[j - 1]) + max(M[i - 1][j - 1], X[i - 1][j - 1], Y[i - 1][j - 1])
            X[i][j] = max(score.gap_start + score.gap + M[i][j - 1], score.gap + X[i][j - 1], score.gap_start + score.gap + Y[i][j - 1])
            Y[i][j] = max(score.gap_start + score.gap + M[i - 1][j], score.gap_start + score.gap + X[i - 1][j], score.gap + Y[i - 1][j])
...
seq1 = 'TATCATATGGT'
seq2 = 'TAGATTTATCAT'
affine_align(seq1,seq2,ScoreParam(gap=-1, match=4, mismatch=-2))
local_align(seq1,seq2,ScoreParam(gap=-1,match=4,mismatch=-2))